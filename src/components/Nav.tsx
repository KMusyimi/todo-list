...
function Render({ projects}: NavProps) {
    ...
    return projects.map((project) => {
        ...
        const style = {
            ...
            backgroundImage: `linear-gradient(90deg, ${rgba} 15%,  rgb(218, 218, 218) 60%,#F7F7F7 100%)`
        } as CSSProperties
        return (
            <li key={id}>
...
export default function Nav({ projectPromise }: { projectPromise: Promise<MyProjects | null> }): JSX.Element {
    ...
    const style = {
        width: '100%',
        backgroundImage: `linear-gradient(90deg, ${hexToRGB('#328E6E', 0.25)} 15%,rgb(218, 218, 218) 60%,#F7F7F7  100%)`
    } as CSSProperties
